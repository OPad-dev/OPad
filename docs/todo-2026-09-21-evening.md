...
| 5.2 | GIF replaced by original artwork generated by `scripts/gen_easter_egg_gif.py` (same 180x93, 14 frames x 90 ms, transparent). |
...
- Git history also covers the GIF (5.2): after replacing it, the old one stays in history unless history is rewritten.
