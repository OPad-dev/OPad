...
- [x] Introduce one predicate (e.g. `DaemonState::may_touch_pad()` = `!foreign_pad && pending_takeover.is_none() && pending_replacement.is_none()`) and use it everywhere a sync/push is triggered: the COOLDOWN→SYNC `TriggerSync` in `runtime.rs`, `ForceSync`, `UpdateConfig`, `SetLayout`/`ResetLayout`, `ResetCounters`, `RestoreDeviceFromPc`, `ImportPcFromDevice`, `ImportBackup`, and `firmware_update::install`'s pre-sync. Handlers return a clear error (`Error("Pad belongs to another install; resolve the takeover prompt first")`).
- [x] Add `foreign_pad: bool` to `IpcResponse::Status` so GUI/CLI can gate. IPC change → bump `IPC_PROTOCOL_VERSION` only if the wire format is not backward compatible under the current serde settings; check how unknown/missing fields are handled and state the decision in the commit.
- [x] After a successful takeover (`ipc_handlers.rs:~708`), also push the custom layouts that `DeviceConnected` suppressed (mirror the `SendLayout` loop at `runtime.rs:~438`).
- [x] `firmware_update::install`: re-check `mode == IDLE` (and `may_touch_pad()`) **immediately before** `pause_and_release`/flash, after the download and stage complete; abort with an error if the user started playing.
...
- [x] `DeviceConnected`: perform the `protocol_version` check **first**. For an incompatible pad: do not set `device_connected`, do not adopt/save its config, do not set `counters_source`; set an `incompatible_device` marker (already exists for the CLI message) and return. `Tick` must not send `RequestDeviceStatus`/`HostStatus`/`TriggerSync` to it.
- [x] `perform_sync` failure path: do not overwrite `st.counters` with the rejected reconciled values. Leave the in-memory counters as the pad reported them (or restore the pre-sync snapshot), keep `esp_counters`/`counters_source` consistent, and make the failure `SyncCompleted` carry the unchanged counters.
...
- [x] `IpcListener::accept`: after `connect()` succeeds, return the connected `server` **even if** creating the replacement instance fails (log it; create lazily on the next `accept`). In the `connect()` error/cancel path, ensure `next` is repopulated so the pipe name always has a listening instance.
...
- [x] AppImage self-replace: on `rename` failing with `EXDEV`, fall back to copy + set permissions + rename within the target dir; don't delete the staged file until the replacement succeeded.
- [x] Run the package-manager `Command` via `tokio::process::Command` (or `spawn_blocking`) so a polkit prompt does not pin a runtime worker.
- [x] `pending_device_push`: either delete the field and its two writes, or have `perform_sync` honour it with `force_restore = true`. Prefer honouring it (the reset is user intent); add a test that a `ResetCounters` while disconnected survives the next connect.
- Verify: `make check`.
...
- [x] Tray: retry the `StatusNotifierWatcher` probe (e.g. every 5 s for the first minute, then every 30 s) or skip the pre-check and let ksni handle late registration; emit `Available` when it appears so the window can hide.
- [x] 🪟 Windows: create the first pipe instance synchronously in `claim()` (before iced starts) so a second launch always finds a listener; surface the error instead of `if let Ok` swallowing it.
- [x] Unix: bind first; only `remove_file` when `connect` fails with `ECONNREFUSED` (stale socket). Test with two near-simultaneous launches.
...
- [x] `monitor --follow`: advance `since_seq` from the last **received** entry's seq (`entries.last()`), never from `latest_seq`; drop the `latest_seq > 0` guard. Fixes both DC#1 and DC#10.
- [x] `--level`/`--source`: derive `clap::ValueEnum` on `LogLevel`/`LogSource` (in `opad-model`) so invalid values error out.
- [x] Filtering: add optional `level`/`source` filters to `GetLogEntries` so the daemon filters before applying `limit`; CLI passes them through. Keep the request backward compatible (optional fields).
- [x] Post-flash verification failure: same exit status with and without daemon — return an error in both branches.
- [x] Incompatibility message: report the device-protocol version the daemon expects (add it to `IncompatibleDevice` or a shared constant in `opad-protocol`), not `IPC_PROTOCOL_VERSION`.
...
- [x] Add an IPC request `ResumeDevice` (resume without waiting for reconnect). `opadctl bootloader` uses it instead of `FinishFlash`; a failed `opadctl flash` also uses it before surfacing the flash error.
- [x] Daemon: track which IPC connection sent `PrepareFlash`; if that connection drops before `FinishFlash`/`ResumeDevice`, call `resume()` automatically and log it.
- [x] CLI: `tokio::signal::ctrl_c` guard around the flash so Ctrl-C sends `ResumeDevice` before exiting.
...
- [x] Extract one `handshake(port, window) -> Option<(HelloAck, Framing)>` used by both `probe_port` and the worker loop. **Behaviour-preserving**: the worker's timing rules win (they are the ones the O9 fix tested); document any difference from the old `probe_port` timing in the commit and confirm `opadctl status`/GUI still finds the pad on cold plug.
...
- [x] .deb: add `libcap2-bin` to `depends` in `desktop/gui/Cargo.toml`; .rpm: add `libcap` to `generate-rpm.requires`; make `test_packages.sh` **not** pre-install `libcap2-bin`/`procps` so the test proves the dependency. Leave `postinst`'s `setcap ... || true` as is (it is correct once the dependency is declared).
- [x] PKGBUILD: run `make espflash` in `build()` and install `/usr/lib/opad/bin/espflash`; add the needed `makedepends`.
- [x] release.yml: include `usr/lib/opad/tosu/tosu` in the glibc assertion with the 2.28 target; add a distro with glibc ≤ 2.31 (e.g. Debian 11) to the matrix or assert via `objdump -T | grep GLIBC_` max version.
...
2026-09-24  S9  bb4f2cc  Claude Opus (Sonnet list)  DG#1 DG#2 DG#4 DG#5 DG#7 fixed; one Page token table for --page and ShowRequested (logs/diagnostics now switch; diagnostics enables its tab); daemon log cursor moves only from replies (last entry received), GUI entries numbered apart, Clear is time-based; adopt_config() shared by Status adopt and ImportCompleted (gameplay_display_hz); designer Applied carries (screen, layout); close while tray probe pending hides, exits if the probe finds no tray; GUI not run by hand
2026-09-24  S10  3ee772f  Claude Opus (Sonnet list)  DG#3 DG#6 DG#8 fixed; Linux tray probed every 5 s for a minute then every 30 s (Unavailable once, Started when it appears; a --tray start's fallback window hides again; a close during the probe waits for the stream's verdict); unix single-instance binds first and unlinks only on ECONNREFUSED; Windows pipe served from its own thread+runtime started in claim() before iced, handoff retried 2 s and errors reported; DG#6 untested on Windows (workspace clippy -D warnings clean for windows-gnu with a stand-in C compiler)
2026-09-24  S11  60b4147  Claude Opus (Sonnet list)  DC#1 DC#4 DC#5 DC#6 DC#7 fixed, DC#10 invalid (LogHub seqs start at 1; guard removed with DC#1 anyway); monitor cursor = last entry received; LogLevel/LogSource ValueEnum behind opad-model feature "clap" (aliases kept, case-insensitive); GetLogEntries gains optional level/source (serde default + skip_serializing_if, no IPC bump), daemon filters before limit, CLI re-filters for older daemons (checked against the running older daemon); post-flash verification error exits non-zero with a daemon too; opad_protocol::DEVICE_PROTOCOL_VERSION shared by daemon checks and the CLI message
2026-09-24  S12  8d37c5d  Claude Opus (Sonnet list)  DC#2 DC#3 fixed; IpcRequest::ResumeDevice/IpcResponse::DeviceResumed (no IPC bump: new variant, CLI falls back to FinishFlash on a new connection with a 2 s cap when an older daemon drops it); opadctl bootloader and a failed flash resume without the 15 s wait; daemon tracks the connection holding a PrepareFlash pause and resumes when it drops; CLI Ctrl-C watcher resumes while the pad is paused, then exits 130; manual bootloader/Ctrl-C test on the pad pending (running daemon is the installed older build)
...
2026-09-24  A8c  (this commit)  Claude Opus  DC#8 follow-up from the user's hardware run: the check only looked in /etc/udev/rules.d while the .deb/.rpm/AppImage/make install put the rule in /usr/lib/udev/rules.d, so a package install would have read as "not installed"; both paths are checked now (an old copy in one does not hide a current one in the other); the user ran the Sep 20 installed opadctl, so step 9 must be re-run after make install-user
2026-09-24  VALIDATION  -  owner  Hardware run on the real pad passed: flash+reconnect (S12), boot (O6/S1/A2), touch as third key (FU#10), keys/remap/detect-pin (O2/O3/O4/S1), full map + replug (O1/S2 pad side), sleep/brightness (S3/FB#1), layout during gameplay + reboot (O5). Everything else signed off by the owner on tests/review without a hardware run (O7 O8 O9 O10 O11 S6 S7 S9 S10 S11 S13 DC#8 DG#10 DD#9). Plan closed; see claude-code-review/README.md "Status".
```
