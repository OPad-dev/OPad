...
2026-09-24  O10  (this commit)  Claude Opus  DD#5 DD#6 fixed (untested on Windows); accept returns the connected client even if the replacement instance fails (lazy recreate), cancel returns the instance to `next`, connect error replenishes; enter_bootloader re-picks after a fired trigger and reports NoBootloader, not PortBusy, when the app port vanished; opad-ipc/opad-device check + clippy clean for x86_64-pc-windows-gnu (full workspace blocked by missing mingw gcc for ring); VM run pending
```
