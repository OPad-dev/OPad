...
2026-09-24  S10  (this commit)  Claude Opus (Sonnet list)  DG#3 DG#6 DG#8 fixed; Linux tray probed every 5 s for a minute then every 30 s (Unavailable once, Started when it appears; a --tray start's fallback window hides again; a close during the probe waits for the stream's verdict); unix single-instance binds first and unlinks only on ECONNREFUSED; Windows pipe served from its own thread+runtime started in claim() before iced, handoff retried 2 s and errors reported; DG#6 untested on Windows (workspace clippy -D warnings clean for windows-gnu with a stand-in C compiler)
```
