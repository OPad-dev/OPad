...
pub const FLUSH_INTERVAL_PLAYING: Duration = Duration::from_millis(100);
...
impl DataSync {
    ...
    pub fn take_changes(&mut self, playing: bool, force: bool) -> Vec<(u8, SourceValue)> {
        let interval = if playing { FLUSH_INTERVAL_PLAYING } else { FLUSH_INTERVAL_IDLE };
...
mod tests {
    ...
    fn only_changes_are_sent() {
        ...
        assert_eq!(sync.take_changes(true, true).len(), 2);

        sync.ingest([(1, SourceValue::Text("a".into())), (20, SourceValue::Number(2.0))]);
        assert_eq!(sync.take_changes(true, true), vec![(20, SourceValue::Number(2.0))]);
    ...
    fn flushes_are_rate_limited() {
        ...
        assert_eq!(sync.take_changes(true, false).len(), 1);
        sync.ingest([(20, SourceValue::Number(2.0))]);
        assert!(sync.take_changes(true, false).is_empty());
    }
}
