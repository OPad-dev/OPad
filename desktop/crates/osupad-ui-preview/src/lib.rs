...
impl From<&RawLayout> for osupad_layout::Layout {
    fn from(raw: &RawLayout) -> Self {
        use osupad_layout::{Align, Font, WidgetKind};
        osupad_layout::Layout {
            background: raw.background,
            widgets: raw.widgets[..raw.count as usize]
                .iter()
                .map(|w| osupad_layout::Widget {
...
pub fn default_model(screen: osupad_layout::Screen) -> osupad_layout::Layout {
...
pub fn render_model(layout: &osupad_layout::Layout) -> Option<Vec<u8>> {
    render(&layout.into())
}
...
mod tests {
    ...
    fn source_ids_match_firmware() {
        use osupad_model::ui_source::ALL;
        for (id, name) in ALL {
