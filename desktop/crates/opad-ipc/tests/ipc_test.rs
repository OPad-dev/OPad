...
use opad_model::{CounterState, DeviceConfig, DeviceInfo, JsonBackup, RuntimeMode};
                ...
                while let Ok(req) = read_request(&mut stream).await {
                    let resp = match req {
                        ...
                        IpcRequest::GetStatus => IpcResponse::Status {
                            ...
                            counters_source: opad_model::CounterSource::Device,
                            pc_counters: None,
                            esp_counters: None,
...
async fn test_connect_and_handshake_helper() {
    ...
    let res = opad_ipc::connect_and_handshake_at(&socket_path).await;
    assert!(res.is_ok());
...
async fn test_oversized_frame_does_not_allocate() {
    ...
    let server_task = tokio::spawn(async move {
        if let Ok(mut stream) = listener.accept().await {
            ...
            match err {
                opad_ipc::IpcError::Protocol(msg) => {
...
fn test_pipe_dacl_is_restricted_to_the_current_user() {
    let sddl = opad_ipc::pipe_security_sddl().expect("pipe_security_sddl");
