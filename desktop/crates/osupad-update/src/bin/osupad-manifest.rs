//! `osupad-manifest` — builds and signs the release manifest (§U-0.3).
//!
//! This is the release-side half of `osupad_update::verify`. It walks a `dist`
//! directory, records every artifact with its SHA-256, writes
//! `osupad-manifest.json`, and produces the detached
//! `osupad-manifest.json.minisig` beside it — the two files
//! `osupad_update::client` fetches.
//!
...
//! # The secret key
//!
//! Lives at `~/.config/osupad/osupad-manifest.key`, outside the repository,
...
//!
//! ```text
//! osupad-manifest --dist dist --base-url https://example.invalid/download \
...
use osupad_update::firmware::FIRMWARE_TARGET;
use osupad_update::manifest::{
    Artifact, ArtifactKind, Component, ReleaseManifest, APP, FIRMWARE, SUPPORTED_SCHEMA, TOSU,
};
use osupad_update::verify::{sha256_file, verify_manifest_signature};
use std::collections::BTreeMap;
use std::path::{Path, PathBuf};
use std::process::Command;

const MANIFEST_NAME: &str = "osupad-manifest.json";

fn main() {
    if let Err(e) = run() {
        eprintln!("osupad-manifest: {e}");
        std::process::exit(1);
    }
}
...
/// `~/.config/osupad/osupad-manifest.key`, honouring `$XDG_CONFIG_HOME`.
///
/// Resolved here rather than in `osupad_model::paths`: that module is the
...
fn default_secret_key() -> PathBuf {
    ...
    base.join("osupad").join("osupad-manifest.key")
}

const HELP: &str = "\
osupad-manifest — build and sign the release manifest (§U-0.3)
  ...
  --secret-key <path>       default: <config dir>/osupad-manifest.key
  --out <path>              default: <dist>/osupad-manifest.json
...
mod tests {
    ...
    fn dist_file_names_map_to_the_right_artifacts() {
        ...
        assert_eq!(app_artifact("osupad-daemon"), None);
    }
    ...
    fn a_built_manifest_round_trips_and_finds_its_artifacts() {
        ...
        assert_eq!(
            fw.sha256,
            osupad_update::verify::sha256_bytes(b"firmware bytes")
        );
        ...
        assert_eq!(
            app.sha256,
            osupad_update::verify::sha256_bytes(b"installer bytes")
        );
    ...
    fn the_signature_sits_beside_the_manifest() {
        assert_eq!(
            signature_path(Path::new("/tmp/dist/osupad-manifest.json")),
            PathBuf::from("/tmp/dist/osupad-manifest.json.minisig")
        );
    }
}
