...
"#,
    ...
    if !reload_status.success() {
        return Err(format!(
            "systemctl --user daemon-reload exited with error code: {:?}",
            reload_status.code()
        ));
    }
    ...
    let enable_status = Command::new("systemctl")
        ...
        .map_err(|e| {
            format!(
                "Failed to run systemctl --user enable --now osupad.service: {}",
                e
            )
        })?;
    ...
    if !enable_status.success() {
        return Err(format!(
            "systemctl --user enable --now osupad.service exited with error code: {:?}",
            enable_status.code()
        ));
    }
