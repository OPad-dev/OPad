...
Boards:
  ...
  HE/he_input_v1           two-key Hall Effect (Rapid Trigger) input module
  Carrier/controller_carrier_v1   Waveshare ESP32-S3-Touch-LCD-2 header carrier (C-shaped)
...
def load_symbols():
    ...
    symbols = {}
    for name in ("R", "C", "SW_Push", "DRV5055", "Conn_01x08", "Conn_01x14"):
...
# -----------------------------------------------------------------------------
# Hall Effect input module
# -----------------------------------------------------------------------------

def build_he():
    """Hall Effect module: generated by scripts/generate_he_kicad.py.

    The HE board is defined once, in ``he_board.py``, because its fabrication
    data is produced without KiCad (see ``generate_he_gerbers.py``).  Keeping a
    second copy of the placement here would let the two drift, so this just
    calls the shared generator and then writes the project files the same way
    the other boards do.  ``he_board.py`` self-checks its design rules and the
    generator refuses to write anything that fails them.
    """
    import generate_he_kicad
    import he_board

    generate_he_kicad.write_schematic()
    generate_he_kicad.write_board()
    directory = os.path.join(V1, "HE")
    write_project(directory, he_board.PROJECT)
    return directory, he_board.PROJECT
...
def build_carrier():
    ...
    directory = os.path.join(V1, "Carrier")
    sheet = project + ".kicad_sch"
...
if __name__ == "__main__":
    for d, p in (build_mx(), build_he(), build_carrier()):
